...
public class JMeterSensor implements Sensor { //, GeneratesViolations {
		...
		try {
			...
			if (summary.getTransTotal()>0) {
				...
				context.saveMeasure(new Measure(
					JMeterMetrics.transMapResponseTimeOkAvg, transMapResponseTimeOkAvgPropBuild.buildData()
				));
				
				// transMapResponseTimeOkDevPercent
				PropertiesBuilder<String, Double> transMapResponseTimeOkDevPropBuild = 
					new PropertiesBuilder<String, Double>(
							JMeterMetrics.transMapResponseTimeOkDevPercent, 
							summary.getTransMapResponseTimeOkAvgDevPercent()
					);
				context.saveMeasure(new Measure(
					JMeterMetrics.transMapResponseTimeOkDevPercent, transMapResponseTimeOkDevPropBuild.buildData()
				));
			}
	...
	/**
	 * Gets a jmeter report client connected to the configured 
	...
	public JMeterReportClient getReportClient(Project project) {
	    ...
	    if (StringUtils.isBlank(port) || Integer.parseInt(port)==0) {
	    	LOG.warn(
	    		"Null or invalid jmeter-report-server PORT. " +
